...
set init_verilog "../syn/results/processor_new.mapped.v"
...
saveDesign db/processor_new_floorplan.enc
...
saveDesign db/processor_new_powerplan.enc
...
saveDesign db/processor_new_place.enc
...
saveDesign db/processor_new_cts.enc
...
saveDesign db/processor_new_postroute.enc
...
verifyGeometry -report reports/processor_new.geometry.rpt
verifyConnectivity -type all -report reports/processor_new.connectivity.rpt
...
report_area -out_file reports/processor_new.routed.area.rpt

# Power report
report_power -outfile reports/processor_new.routed.power.rpt

# Setup Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -late > processor_new_setup.mtarpt

# Hold Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -early > processor_new_hold.mtarpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/processor_new.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/processor_new.sdf

# *.v: verilog netlist file
saveNetlist results/processor_new.routed.v

# *.gds: GDS file
streamOut results/processor_new.gds -mapFile streamOut.map \
